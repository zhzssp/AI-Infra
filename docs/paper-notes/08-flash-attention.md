...
> **导航**：[笔记索引](README.md) · [自学枢纽](../README.md)（阶段 5，自选补充；建议穿插进阶段 4 开头） · [横切概念](../learning-guides/ai-compiler-foundations-learning-guide.md) §4.3（Roofline）、§9.4（IO 感知方法论）  
> **在学习路线中的定位（自选补充）**：本篇**不在主链上**，也没有配套动手项目。它以一个身份融入——**「融合 + tiling + 重计算」的实证**，用来在学 TVM/schedule 之前建立「为什么值得做这些变换」的动机。  
> 结论已蒸馏进 [`../learning-guides/ai-compiler-foundations-learning-guide.md`](../learning-guides/ai-compiler-foundations-learning-guide.md) §4.3 / §9.4，**跳过本篇不断链**。要读就抓「HBM 访存是瓶颈」这条主线，**online softmax 与 IO 复杂度公式看懂即可，不必手推**。  
