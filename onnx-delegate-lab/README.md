...
> **不要与 `tvm-fatbin-lab` 合并**：那边回答「划完怎么算得快 / 多变体打包」；这边回答「谁来划分、边界付什么代价」。同构，分层不同——见 [`docs/ai-compiler-foundations.md`](../docs/ai-compiler-foundations.md) §3.3–3.4。
...
| **配套教材** | [`../docs/onnx-learning-guide.md`](../docs/onnx-learning-guide.md) · [`../docs/executorch-learning-guide.md`](../docs/executorch-learning-guide.md) |
| **横切词汇** | [`../docs/ai-compiler-foundations.md`](../docs/ai-compiler-foundations.md) §3.3（四类边界代价）· §3.4（四栈选型） |
...
| 步骤 | 核心概念 | 产物 |
|------|----------|------|
| **O01** | Model/Graph/Node；initializer vs runtime input；shape infer | `out/onnx/01_*` |
| **O02** | helper 插节点 / 改 initializer；checker | `out/onnx/02_*` |
| **O03** | ORT providers；分区观察；四类边界代价 | `out/onnx/03_*` |
| **E01** | Partitioner 契约；tag 粒度 ↔ 边界数 | `out/executorch/01_*` |
| **E02** | ExecuTorch / ORT / IREE 三系统对照 | `out/executorch/02_THREE_SYSTEMS.md` |

```
同一小模型
  ├─ ORT：Session 构建期多 EP 协商划分
  ├─ ExecuTorch：导出期 Partitioner 打 tag
  └─ IREE（对照教材）：编译期形成 flow.dispatch
```
...
├── executorch_lab/
│   ├── 01_partitioner_lab.py    # 真实 ET 或概念模拟
